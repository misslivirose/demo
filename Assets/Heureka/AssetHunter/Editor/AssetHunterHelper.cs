...
namespace HeurekaGames
{
    internal class AssetHunterHelper
    {
        ...
        private static void traverseDirectory(int parentIndex, string path, List<BuildReportAsset> usedAssets, int heirarchyDepth, ref int directoriesTraversed, SortedDictionary<SerializableSystemType, bool> validTypeList)
        {
            ...
            if (parentIndex != -1)
            {
                AssetHunterWindow.Instance.GetFolderList()[parentIndex].AddChildFolder(afInfo);
            }
            ...
            foreach (string assetName in result)
            {
                ...
                if (assetType.SystemType != typeof(MonoScript))
                {
                    AssetObjectInfo newAssetInfo = new AssetObjectInfo(assetName, /*objToFind,*/ assetType);
            ...
            {
                traverseDirectory(AssetHunterWindow.Instance.GetFolderList().IndexOf(afInfo), nld, usedAssets, (heirarchyDepth + 1), ref directoriesTraversed, validTypeList);
            }
        }
    }
}